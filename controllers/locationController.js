...
const createSACRecord = async (req, res) => {
  const { name, roll_no, email, mobile_number, room_no, hostel } = req.body;
  //Todo handle description
  console.log("SAC record request came");
  const description = "";
  try {
    ...
    console.error("Error creating SAC record: " + error.stack);
    res.status(500).send("Error creating SAC record.");
  }
};
...
const updateSACStatus = async (req, res) => {
  try {
    ...
    const current_in = previousCurrentIn - 1;
    ...
    return res.status(200).json({
      message: "SAC exit status updated successfully!",
    });
  } catch (error) {
    console.error("Error updating SAC exit status: " + error.stack);
    return res.status(500).send({
      message: "Error updating SAC exit status.",
    });
  }
};
...
const createLibraryRecord = async (req, res) => {
  ...
  try {
    ...
    if (prevStatus == -1 || prevStatus === 0) {
      ...
      await updateLibraryStatus(req, res);
    }
  } catch (error) {
    console.error("Error creating Library record: " + error.stack);
    res.status(500).send("Error creating Library record.");
  }
};
...
const updateLibraryStatus = async (req, res) => {
  try {
    ...
    await query(exitLibraryQuery, values, res);
    res.status(200).send("Library exit status updated successfully!");
  } catch (error) {
    console.error("Error updating Library exit status: " + error);
    res.status(500).send("Error updating Library exit status.");
  }
};
...
const getCurrentInValue = async (req, res) => {
  try {
    ...
    console.error("Error retrieving current_in value: " + error.stack);
    res.status(500).send("Error retrieving current_in value.");
  }
};
